...
//                    method = methodNow;
        ...
        if (lang != null) {
            if (lang.toLowerCase().equals(lang) && lang.length() == 2)
    ...
    private void smartInvoke(Method method, Object page, HttpServletRequest request, Map<String, Object> view)
            throws InvocationTargetException, IllegalAccessException {
        List<String> parameterTypes = Arrays.stream(method.getParameterTypes()).map(Class::getSimpleName).collect(Collectors.toList());
        if (!parameterTypes.contains("Map") && !parameterTypes.contains("HttpServletRequest")) {
            method.invoke(page);
        } else if (parameterTypes.contains("Map") && parameterTypes.contains("HttpServletRequest")) {
            method.invoke(page, request, view);
        } else {
            if (parameterTypes.contains("Map")) {
                method.invoke(page, view);
            } else if (parameterTypes.contains("HttpServletRequest")) {
    ...
    private Template newTemplate(String templateName, String lang) throws ServletException {
        ...
        if (sourceConfiguration != null) {
            try {
                template = sourceConfiguration.getTemplate(newTemplateName);
            } catch (TemplateNotFoundException ignored) {
                // No operations.
            } catch (IOException e) {
                throw new ServletException("Can't load template [templateName=" + newTemplateName + "]", e);
            }
        }

        if (template == null && targetConfiguration != null) {
            try {
                template = targetConfiguration.getTemplate(newTemplateName);
            } catch (TemplateNotFoundException ignored) {
                // No operations.
            } catch (IOException e) {
                throw new ServletException("Can't load template [templateName=" + newTemplateName + "]", e);
